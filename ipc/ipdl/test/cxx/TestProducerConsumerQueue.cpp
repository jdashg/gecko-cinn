...
void
Produce(UniquePtr<Producer>&& producer)
{
...
void
TestProducerConsumerQueueParent::Main()
{
...
int
ConsumeInt(UniquePtr<Consumer>& consumer)
{
    TimeStamp start = TimeStamp::Now();
    TimeDuration maxWait = TimeDuration::FromMilliseconds(MAX_MS_WAIT);
    PcqStatus status;
...
void
Consume(UniquePtr<Consumer>&& consumer)
{
    const int expected[] = { 1, 2, 3 };
    for (uint32_t i=0; i<arraysize(expected); ++i) {
        int found = ConsumeInt(consumer);
        if (expected[i] != found) {
            fail("incorrect value obtained in deserialization.  "
                 "Expected: %d.  Found: %d", expected[i], found);
        }
    }
...
TestProducerConsumerQueueChild::RecvConsume(UniquePtr<Consumer>&& aConsumer)
{
