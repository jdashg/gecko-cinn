...
void VRLayerChild::SubmitFrame(const VRDisplayInfo& aDisplayInfo) {
  ...
  if (kIsAndroid) {
    ...
    getNewFrame = (!mThisFrameTextureDesc || aDisplayInfo.mDisplayState.lastSubmittedFrameId ==
                                  mLastSubmittedFrameId);
  }
  if (getNewFrame) {
    webgl::Present(*webgl);

    RefPtr<layers::ImageBridgeChild> imageBridge = layers::ImageBridgeChild::GetSingleton();
