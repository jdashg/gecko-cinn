...
void CanvasClient::UseTexture(TextureClient* const aTexture, const wr::RenderRoot aRenderRoot) {
  MOZ_ASSERT(aTexture);
...
RefPtr<TextureClient> CanvasClient::CreateTextureClientForCanvas(
    const gfx::SurfaceFormat aFormat, const gfx::IntSize aSize, const TextureFlags aFlags) {
  if (kIsWindows) {
