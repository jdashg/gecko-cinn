...
/*static*/
UniquePtr<SharedSurface_D3D11Interop> SharedSurface_D3D11Interop::Create(const SharedSurfaceDesc& desc,
  ...
  auto hr = d3d->CreateTexture2D(&texDesc, nullptr, getter_AddRefs(data.texD3D));
  if (FAILED(hr)) {
  ...
  hr = data.texD3D->QueryInterface(__uuidof(IDXGIResource), getter_AddRefs(texDXGI));
  if (FAILED(hr)) {
  ...
  data.lockHandle =
      interop->RegisterObject(data.texD3D, data.interopRb->name, LOCAL_GL_RENDERBUFFER,
  ...
  auto fbForDrawing = MozFramebuffer::CreateForBacking(gl, size, 0, false, LOCAL_GL_RENDERBUFFER, data.interopRb->name);
  ...
  return AsUnique(new SharedSurface_D3D11Interop(desc, std::move(fbForDrawing), std::move(data)));
}

SharedSurface_D3D11Interop::SharedSurface_D3D11Interop(const SharedSurfaceDesc& desc,
  UniquePtr<MozFramebuffer>&& fbForDrawing, Data&& data)
...
void SharedSurface_D3D11Interop::ProducerReleaseImpl() {
  ...
  if (mData.interopFbIfNeedsIndirect) {
    ...
    gl->BlitHelper()->DrawBlitTextureToFramebuffer(mFb->ColorTex(), mDesc.size, mDesc.size);
  }
...
Maybe<layers::SurfaceDescriptor> SharedSurface_D3D11Interop::ToSurfaceDescriptor() {
  const auto format = gfx::SurfaceFormat::B8G8R8A8;
  return Some(layers::SurfaceDescriptorD3D10(
      WindowsHandle(mData.dxgiHandle), format, mDesc.size, gfx::YUVColorSpace::UNKNOWN,
      gfx::ColorRange::FULL));
}
...
UniquePtr<SurfaceFactory_D3D11Interop> SurfaceFactory_D3D11Interop::Create(
  ...
  return AsUnique(new SurfaceFactory_D3D11Interop({&gl, SharedSurfaceType::DXGLInterop2, layers::TextureType::D3D11,
    true}, interop));
}
