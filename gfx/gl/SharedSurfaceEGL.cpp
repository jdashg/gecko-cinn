...
/*static*/
UniquePtr<SharedSurface_EGLImage> SharedSurface_EGLImage::Create(const SharedSurfaceDesc& desc) {
  ...
  const auto buffer =
      reinterpret_cast<EGLClientBuffer>(fb->ColorTex());
...
SharedSurface_EGLImage::SharedSurface_EGLImage(const SharedSurfaceDesc& desc,
                          UniquePtr<MozFramebuffer>&& fb, const EGLImage image)
...
Maybe<layers::SurfaceDescriptor> SharedSurface_EGLImage::ToSurfaceDescriptor() {
  return Some(layers::EGLImageDescriptor(
      (uintptr_t)mImage, (uintptr_t)mSync, mDesc.size, true));
}
...
SurfaceFactory_SurfaceTexture::SurfaceFactory_SurfaceTexture(GLContext& gl)
    : SurfaceFactory({&gl,
                      ...
                      layers::TextureType::AndroidNativeWindow,
                      true}) {}
