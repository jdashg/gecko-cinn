                        ...
                        const OriginPos destOrigin) const
{
    ...
    const ScopedBindTexture savedTex(mGL, 0, LOCAL_GL_TEXTURE_EXTERNAL);
    ...
    const bool flipY = (destOrigin != srcOrigin);

    // There is no padding on these images, so we can use the GetTransformMatrix directly.
    const DrawBlitProg::BaseArgs baseArgs = { transform3, flipY, destSize, Nothing() };
