                                       ...
                                       const gfx::IntSize& size)
{
    ...
    const auto& pbuffer = egl->fCreatePbufferSurface(display, config, attribs);
    if (!pbuffer)
        return nullptr;
...
D3DContext(GLLibraryEGL* const egl)
{
...
void
SharedSurface_ANGLEShareHandle::UnlockProd()
{
	// Don't bother to restore.
}
...
{
    if (mAngleKeyedMutex) {
        const auto hr = mAngleKeyedMutex->AcquireSync(0, 10000);
        if (hr == WAIT_TIMEOUT) {
...
bool
SharedSurface_ANGLEShareHandle::ReadbackBySharedHandle(gfx::DataSourceSurface* const out_surface)
{
