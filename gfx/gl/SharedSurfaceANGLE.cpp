...
static EGLSurface CreatePBufferSurface(GLLibraryEGL* egl, EGLDisplay display,
  ...
  const EGLint attribs[] = {LOCAL_EGL_WIDTH, size.width, LOCAL_EGL_HEIGHT, size.height,
                      LOCAL_EGL_NONE};
...
SharedSurface_ANGLEShareHandle::Create(const SharedSurfaceDesc& desc) {
  ...
  return AsUnique(new SharedSurface_ANGLEShareHandle(desc, egl, pbuffer, shareHandle, keyedMutex));
}

EGLDisplay SharedSurface_ANGLEShareHandle::Display() const { return mEGL->Display(); }

SharedSurface_ANGLEShareHandle::SharedSurface_ANGLEShareHandle(const SharedSurfaceDesc& desc,
    GLLibraryEGL* egl,
...
Maybe<layers::SurfaceDescriptor> SharedSurface_ANGLEShareHandle::ToSurfaceDescriptor() {
  const auto format = gfx::SurfaceFormat::B8G8R8A8;
  return Some(layers::SurfaceDescriptorD3D10(
      (WindowsHandle)mShareHandle, format, mDesc.size, gfx::YUVColorSpace::UNKNOWN,
      gfx::ColorRange::FULL));
}
...
SurfaceFactory_ANGLEShareHandle::Create(
  ...
  MOZ_ASSERT(dm);
  if (gl.IsWARP() != dm->IsWARP() ||
      !dm->TextureSharingWorks()) {
    return nullptr;
  }

  return AsUnique(new SurfaceFactory_ANGLEShareHandle({&gl, SharedSurfaceType::EGLSurfaceANGLE, layers::TextureType::D3D11, true}));
}
