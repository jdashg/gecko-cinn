...
already_AddRefed<GLContextEGL> GLContextEGL::CreateGLContext(
    GLLibraryEGL* const egl, const GLContextDesc& desc, EGLConfig config, EGLSurface surface, const bool useGles,
...
static EGLConfig ChooseConfig(GLLibraryEGL* const egl, const GLContextCreateDesc& desc, const bool useGles) {
  nsTArray<EGLint> configAttribList;
  FillContextAttribs(
      bool(desc.flags & CreateContextFlags::PREFER_ES3), useGles, &configAttribList);

  const EGLint* configAttribs = configAttribList.Elements();

  // The sorting dictated by the spec for eglChooseConfig reasonably assures that a reasonable 'best' config
  // is on top.
...
/*static*/
already_AddRefed<GLContextEGL> GLContextEGL::CreateEGLPBufferOffscreenContextImpl(
    const GLContextCreateDesc& desc, const mozilla::gfx::IntSize& size, const bool useGles,
    nsACString* const out_failureId) {
...
already_AddRefed<GLContextEGL> GLContextEGL::CreateEGLPBufferOffscreenContext(
  ...
  if (!gl) {
    gl = CreateEGLPBufferOffscreenContextImpl(
        desc, size, /* useGles */ true, out_failureId);
  }
  return gl.forget();
}
...
already_AddRefed<GLContext> GLContextProviderEGL::CreateOffscreen(
    const mozilla::gfx::IntSize& size, const GLContextCreateDesc& desc, nsACString* const out_failureId) {
