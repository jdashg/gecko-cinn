...
SharedSurface::SharedSurface(const SharedSurfaceDesc& desc, UniquePtr<MozFramebuffer> fb)
...
/* static */
UniquePtr<SurfaceFactory> SurfaceFactory::Create(GLContext* const pGl,
  ...
  case layers::TextureType::WaylandDMABUF:
    ...
    if (!gfxPlatformGtk::GetPlatform()->UseWaylandDMABufWebGL()) return nullptr;
  ...
  case layers::TextureType::AndroidNativeWindow:
    ...
    if (XRE_IsParentProcess() && !StaticPrefs::webgl_enable_surface_texture()) return nullptr;
