                       ...
                       const layers::TextureFlags flags)
{
    ...
    case mozilla::layers::LayersBackend::LAYERS_OPENGL:
        ...
        factory = AsUnique(new SurfaceFactory_IOSurface(gl, depthStencil, ipcChannel,
                                                        flags));
#elif defined(GL_PROVIDER_GLX)
        factory = SurfaceFactory_GLXDrawable::Create(gl, depthStencil, ipcChannel, flags);
#elif defined(MOZ_WIDGET_UIKIT)
        factory = MakeUnique<SurfaceFactory_GLTexture>(gl, depthStencil, ipcChannel,
                                                       flags);
#else
        if (gl->GetContextType() == GLContextType::EGL) {
            if (XRE_IsParentProcess()) {
                factory = SurfaceFactory_EGLImage::Create(gl, depthStencil, ipcChannel,
                                                          flags);
            }
        }
#endif
...
#ifdef XP_WIN
        factory = SurfaceFactory_ANGLEShareHandle::Create(gl, depthStencil, ipcChannel,
                                                          flags);

        if (!factory) {
            factory = SurfaceFactory_D3D11Interop::Create(gl, depthStencil, ipcChannel,
                                                          flags);
        }
#endif
        break;
