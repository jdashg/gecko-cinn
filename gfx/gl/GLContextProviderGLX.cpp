...
GLXLibrary::EnsureInitialized()
{
    ...
    PRLibrary* lib = nullptr;
    {
        ...
        lib = PR_LoadLibrary(libGLfilename);
        if (!lib) {
    ...
    *(PRFuncPtr*)&mGetProcAddress = PR_FindFunctionSymbol(lib, "glXGetProcAddress");
    if (!mGetProcAddress) {
        NS_ERROR("Failed to locate glXGetProcAddress. Is GLX older than 1.4?");
        return false;
    }
...
#define END_OF_SYMBOLS { nullptr, { nullptr } }
    ...
    const SymLoadStruct symbols_swapcontrol[] = {
...
#undef END_OF_SYMBOLS
    ...
    const auto fnLoadSymbols = [&](const SymLoadStruct* list) {
        return LoadSymbols(mGetProcAddress, list, true);
    };

    if (!fnLoadSymbols(symbols)) {
        NS_ERROR("Couldn't find required GLX entry points.");
        return false;
    }
    ...
    if (!xQueryVersion(display, &majorVersion, &minorVersion) ||
        majorVersion != 1 ||
        minorVersion < 4)
    {
        NS_ERROR("Requires GLX 1.4 (from 2005)");
        return false;
    }

    const auto& clientVendor = xGetClientString(display, LOCAL_GLX_VENDOR);
    const auto& serverVendor = xQueryServerString(display, screen, LOCAL_GLX_VENDOR);
    const auto& extensionsStr = xQueryExtensionsString(display, screen);
    ...
    {
        (void)fnLoadSymbols(symbols_createcontext);
    }
...
{
    BEFORE_GLX_CALL;
    const auto result = mSymbols.fCreatePixmap(display, config, pixmap, attrib_list);
    AFTER_GLX_CALL;
    return result;
}
...
GLContextGLX::GLContextGLX(
                  ...
                  ContextProfile profile)
    : GLContext(sGLXLibrary.mGetProcAddress, flags, caps, shareContext, isOffscreen),
      mContext(aContext),
      mDisplay(aDisplay),
      mDrawable(aDrawable),
                                    ...
                                    bool aWebRender)
{
    ...
    if (aWebRender) {
        ...
        cfgs = sGLXLibrary.xChooseFBConfig(display, screen, webrenderAttribs,
                                           &numConfigs);
    } else {
        cfgs = sGLXLibrary.xGetFBConfigs(display, screen, &numConfigs);
    }
                             ...
                             ContextProfile profile = ContextProfile::OpenGLCompatibility)
{
    ...
    do {
        ...
        const RefPtr<gfxXlibSurface> surface = gfxXlibSurface::Create(DefaultScreenOfDisplay(display),
                                                                      visual, dummySize);
