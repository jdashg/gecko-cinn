...
TreeWalker::ParentNode(ErrorResult& aResult)
{
    ...
    while (node && node != mRoot) {
        ...
        if (node) {
            ...
            if (filtered == bindings::NodeFilter::FILTER_ACCEPT) {
                mCurrentNode = node;
                return node.forget();
            }
        }
...
TreeWalker::PreviousNode(ErrorResult& aResult)
{
    ...
    while (node != mRoot) {
        while (nsINode *previousSibling = node->GetPreviousSibling()) {
            ...
            nsINode *lastChild;
            while (filtered != bindings::NodeFilter::FILTER_REJECT &&
...
{
    int16_t filtered = bindings::NodeFilter::FILTER_ACCEPT; // pre-init for inner loop
...
TreeWalker::FirstChildInternal(bool aReversed, ErrorResult& aResult)
{
    ...
    while (node) {
        ...
        switch (filtered) {
            case bindings::NodeFilter::FILTER_ACCEPT:
                // Node found
                mCurrentNode = node;
                return node.forget();
            case bindings::NodeFilter::FILTER_SKIP: {
            ...
            case bindings::NodeFilter::FILTER_REJECT:
                // Keep searching
                break;
        }
...
TreeWalker::NextSiblingInternal(bool aReversed, ErrorResult& aResult)
{
    ...
    while (1) {
        ...
        while (sibling) {
            ...
            if (filtered == bindings::NodeFilter::FILTER_REJECT ||
...
{
    return bindings::TreeWalker::Wrap(aCx, this, aGivenProto, aReflector);
}
