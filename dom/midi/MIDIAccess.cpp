...
MIDIAccess::FireConnectionEvent(MIDIPort* aPort)
{
  ...
  if (aPort->State() == MIDIPortDeviceState::Disconnected) {
    if (aPort->Type() == MIDIPortType::Input &&
        bindings::MIDIInputMap::MaplikeHelpers::Has(mInputMap, id, rv)) {
      bindings::MIDIInputMap::MaplikeHelpers::Delete(mInputMap, id, rv);
    } else if (aPort->Type() == MIDIPortType::Output &&
               bindings::MIDIOutputMap::MaplikeHelpers::Has(mOutputMap, id, rv)) {
      bindings::MIDIOutputMap::MaplikeHelpers::Delete(mOutputMap, id, rv);
    }
    ...
    if (aPort->Type() == MIDIPortType::Input &&
      ...
      bindings::MIDIInputMap::MaplikeHelpers::Set(mInputMap,
                                               id,
      ...
      bindings::MIDIOutputMap::MaplikeHelpers::Set(mOutputMap,
                                                id,
...
MIDIAccess::MaybeCreateMIDIPort(const MIDIPortInfo& aInfo, ErrorResult& aRv)
{
  ...
  if (type == MIDIPortType::Input) {
    bool hasPort = bindings::MIDIInputMap::MaplikeHelpers::Has(mInputMap, id, aRv);
    ...
    bool hasPort = bindings::MIDIOutputMap::MaplikeHelpers::Has(mOutputMap, id, aRv);
    if (hasPort || NS_WARN_IF(aRv.Failed())) {
      // We already have the port in our map.
      return;
    }
...
{
  return bindings::MIDIAccess::Wrap(aCx, this, aGivenProto);
}
