...
static webgl::Limits MakeLimits(const WebGLContext& webgl) {
  ...
  gl.GetUIntegerv(LOCAL_GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS, &limits.maxTexUnits);
  ...
  if (webgl.IsWebGL2()) {
    gl.GetUIntegerv(LOCAL_GL_MAX_ARRAY_TEXTURE_LAYERS, &limits.maxTexArrayLayers);
    gl.GetUIntegerv(LOCAL_GL_MAX_3D_TEXTURE_SIZE, &limits.maxTex3dSize);
    gl.GetUIntegerv(LOCAL_GL_MAX_TRANSFORM_FEEDBACK_SEPARATE_ATTRIBS,
              &limits.maxTransformFeedbackSeparateAttribs);
    gl.GetUIntegerv(LOCAL_GL_MAX_UNIFORM_BUFFER_BINDINGS, &limits.maxUniformBufferBindings);
    gl.GetUIntegerv(LOCAL_GL_UNIFORM_BUFFER_OFFSET_ALIGNMENT,
              &limits.uniformBufferOffsetAlignment);
  }

  if (limits.supportedExtensions[WebGLExtensionID::WEBGL_compressed_texture_astc]) {
    limits.astcHdr = gl.IsExtensionSupported(gl::GLContext::KHR_texture_compression_astc_hdr);
  }

  if (webgl.IsWebGL2() || limits.supportedExtensions[WebGLExtensionID::WEBGL_draw_buffers]) {
...
bool WebGLContext::InitAndValidateGL(FailureReason* const out_failReason) {
  ...
  */
  ...
  gl->GetUIntegerv(LOCAL_GL_MAX_RENDERBUFFER_SIZE,
  ...
  if (StaticPrefs::webgl_min_capability_mode()) {
    ...
    ok &= RestrictCap(&limits.maxTexUnits,
    ...
    ok &= RestrictCap(&limits.lineWidthRange[1], kCommonAliasedLineWidthRangeMax);
    ok &= RestrictCap(&limits.pointSizeRange[1], kCommonAliasedPointSizeRangeMax);
