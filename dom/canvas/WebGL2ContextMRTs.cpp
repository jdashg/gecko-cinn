...
bool WebGL2Context::ValidateClearBuffer(const GLenum buffer, const GLint drawBuffer,
...
void WebGL2Context::ClearBufferTv(GLenum buffer, GLint drawBuffer,
  ...
  switch (data.type) {
    ...
    case webgl::AttribBaseType::Float:
      gl->fClearBufferfv(buffer, drawBuffer, reinterpret_cast<const float*>(data.data));
      break;

    case webgl::AttribBaseType::Int:
      gl->fClearBufferiv(buffer, drawBuffer, reinterpret_cast<const int32_t*>(data.data));
      break;

    case webgl::AttribBaseType::Uint:
      gl->fClearBufferuiv(buffer, drawBuffer, reinterpret_cast<const uint32_t*>(data.data));
      break;
  }
}
