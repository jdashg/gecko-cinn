                        ...
                        new WidgetPointerEvent(false, eVoidEvent, nullptr))
{
  ...
  if (aEvent) {
    ...
    mouseEvent->inputSource = bindings::MouseEvent::MOZ_SOURCE_UNKNOWN;
  }
...
{
  return bindings::PointerEvent::Wrap(aCx, this, aGivenProto);
}
...
ConvertStringToPointerType(const nsAString& aPointerTypeArg)
{
  if (aPointerTypeArg.EqualsLiteral("mouse")) {
    return bindings::MouseEvent::MOZ_SOURCE_MOUSE;
  }
  if (aPointerTypeArg.EqualsLiteral("pen")) {
    return bindings::MouseEvent::MOZ_SOURCE_PEN;
  }
  if (aPointerTypeArg.EqualsLiteral("touch")) {
    return bindings::MouseEvent::MOZ_SOURCE_TOUCH;
  }

  return bindings::MouseEvent::MOZ_SOURCE_UNKNOWN;
}
...
ConvertPointerTypeToString(uint16_t aPointerTypeSrc, nsAString& aPointerTypeDest)
{
  switch (aPointerTypeSrc) {
    case bindings::MouseEvent::MOZ_SOURCE_MOUSE:
      aPointerTypeDest.AssignLiteral("mouse");
      break;
    case bindings::MouseEvent::MOZ_SOURCE_PEN:
      aPointerTypeDest.AssignLiteral("pen");
      break;
    case bindings::MouseEvent::MOZ_SOURCE_TOUCH:
