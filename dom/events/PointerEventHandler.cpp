...
PointerEventHandler::UpdateActivePointerState(WidgetMouseEvent* aEvent)
{
  ...
  case ePointerUp:
    ...
    if (WidgetPointerEvent* pointerEvent = aEvent->AsPointerEvent()) {
      if(pointerEvent->inputSource != bindings::MouseEvent::MOZ_SOURCE_TOUCH) {
        sActivePointersIds->Put(pointerEvent->pointerId,
...
{
  MOZ_ASSERT(aContent);
  if (bindings::MouseEvent::MOZ_SOURCE_MOUSE == GetPointerType(aPointerId)) {
                       ...
                       bool aIsPrimary)
{
  ...
  aPointerEvent->inputSource = bindings::MouseEvent::MOZ_SOURCE_TOUCH;
}
