...
void nsMenuBarListener::InitAccessKey()
{
  ...
  mAccessKeyMask = 0;
#else
  mAccessKey = dom::bindings::KeyboardEvent::DOM_VK_ALT;
  ...
  switch (mAccessKey) {
  case dom::bindings::KeyboardEvent::DOM_VK_SHIFT:
...
nsMenuBarListener::KeyDown(Event* aKeyEvent)
{
  ...
  bool capturing = (eventPhase == dom::bindings::Event::CAPTURING_PHASE);
...
nsMenuBarListener::MouseDown(Event* aMouseEvent)
{
  ...
  if (aMouseEvent->EventPhase() == dom::bindings::Event::CAPTURING_PHASE) {
    return NS_OK;
  }
