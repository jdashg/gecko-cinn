                                           ...
                                           Selection* aSel, int16_t aReason)
{
  ...
  if (sHideCaretsForMouseInput &&
      mLastInputSource == bindings::MouseEvent::MOZ_SOURCE_MOUSE) {
    HideCarets();
    return NS_OK;
  }
  ...
  if (sHideCaretsForMouseInput &&
      mLastInputSource == bindings::MouseEvent::MOZ_SOURCE_KEYBOARD &&
  ...
  int32_t* aOutContentOffset) const
{
  ...
  if (!startFrame) {
    ...
    RefPtr<TreeWalker> walker = mPresShell->GetDocument()->CreateTreeWalker(
      *startNode, dom::bindings::NodeFilter::SHOW_ALL, nullptr, err);
